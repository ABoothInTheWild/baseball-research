# -*- coding: utf-8 -*-
"""
Created on Thu Jul 11 12:55:12 2019

@author: ABooth
"""

#2019 Preseason Playoff Odds
import heapq
...
for league in DivisionsLeague:
    ...
    for div in league:
        ...
        tempResults = []
        for team in div:
            alphaEst = beta19Pre[beta19Pre.Team_Abbr == team]["PriorAlpha"].values[0]
            betaEst = beta19Pre[beta19Pre.Team_Abbr == team]["PriorBeta"].values[0]
...
resultsDF["DivisionOdds20190327"] = divOdds
resultsDF["WildCardOdds20190327"] = wcOdds
resultsDF["PlayoffOdds20190327"] = resultsDF.DivisionOdds20190327 + resultsDF.WildCardOdds20190327
resultsDF["ExpectedWins20190327"] = expectedWins

#Attach point win estimates and confidence itervals
#resultsDF = resultsDF.sort_values(by=["Teams"]).reset_index(drop=True)
#beta19PreSorted = beta19Pre.sort_values(by=["Team_Abbr"]).reset_index(drop=True)
#resultsDF_Full = pd.concat([resultsDF, beta19PreSorted.iloc[:,19:37]], axis=1)

resultsDF.to_csv("mlb2019PreseasonPlayoffPreds.csv", index=False)
###############################################################################
...
resultsDF.to_csv("mlb2019PlayoffPreds.csv", index=False)

#######################################################################

#Playoff Odds

resultsDF = pd.read_csv("mlb2019PlayoffPreds.csv")
    ...
    for division in league:
        for dataType in dataTypes:
            ...
            if dataType != 'ExpectedWins':
                plotTitle = divisionName + ' 2019 Bayesian ' + dataType + ' Probabilities'
                yLabel = dataType + ' Probability'
                fileName = os.getcwd() + fileNamePrefix + divisionName + '_2019_' + dataType + '_Probs'
                yStart = 0
                yEnd = 1.05
                hoverFormat = '.2f'
            else:
                plotTitle = divisionName + ' 2019 Bayesian Expected Wins'
                yLabel = "Expected Wins"
                fileName = os.getcwd() + fileNamePrefix + divisionName + '_2019_' + dataType
                yStart = 45
                yEnd = 120
                hoverFormat = '.0f'
            ...
            for teamAbbr in teamHeaders:
                ...
                layout = go.Layout(
                    ...
                    xaxis = dict(title = '',
                               range = [to_unix_time(datetime.datetime(2019, 3, 20)),
                                        to_unix_time(datetime.datetime(2019, 7, 22))]))
...
for league in LeagueTeams:
    for dataType in dataTypes:
        ...
        if dataType != 'ExpectedWins':
            plotTitle = leagueName + ' 2019 Bayesian ' + dataType + ' Probabilities'
            yLabel = dataType + ' Probability'
            fileName = os.getcwd() + fileNamePrefix + leagueName + '_2019_' + dataType + '_Probs'
            yStart = 0
            yEnd = 1.05
            hoverFormat = '.2f'
        else:
            plotTitle = leagueName + ' 2019 Bayesian Expected Wins'
            yLabel = "Expected Wins"
            fileName = os.getcwd() + fileNamePrefix + leagueName + '_2019_' + dataType
            yStart = 45
            yEnd = 120
            hoverFormat = '.0f'
...
for dataType in dataTypes:
    ...
    if dataType != 'ExpectedWins':
        plotTitle = levName + ' 2019 Bayesian ' + dataType + ' Probabilities'
        yLabel = dataType + ' Probability'
        fileName = os.getcwd() + fileNamePrefix + levName + '_2019_' + dataType + '_Probs'
        yStart = 0
        yEnd = 1.05
        hoverFormat = '.2f'
    else:
        plotTitle = levName + ' 2019 Bayesian Expected Wins'
        yLabel = "Expected Wins"
        fileName = os.getcwd() + fileNamePrefix + levName + '_2019_' + dataType
        yStart = 45
        yEnd = 120
        hoverFormat = '.0f'
