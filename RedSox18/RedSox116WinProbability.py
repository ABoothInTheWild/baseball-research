...
#read data
os.chdir('C:/Users/Alexander/Documents/Baseball/RedSox18')
...
beatFranchiseRecordProb.append(prob)
...
x = dates
fileName = os.getcwd() + '/RedSox106WinProbability'
hoverFormat = '.2f'
data = []
trace = go.Scatter(
        x=x,
        y=beatFranchiseRecordProb,
        mode='lines+markers',
        name = team,
        line = dict(
                color = teamColors[team],
...
data.append(trace)    
layout = go.Layout(
    title = 'Probability the Boston Red Sox Win at least 106 Games in 2018',
    yaxis = dict(title = 'Probability',
                 range = [0, 1.05],
                 hoverformat = hoverFormat),
    xaxis = dict(title = '',
...
ax.set_ylim([0, 13])
plt.title(team + " " + currDate + ' WP% Posterior Beta Estimate')
plt.ylabel('Density')
plt.xlabel('Winning Percentage')
plt.grid(b=True, which='major', color='gray', linestyle='--', alpha= 0.3)
plt.show()
fig.savefig(team+currDate + 'Posterior.png', bbox_inches='tight')
...
ax.set_xlim([0.25, 0.75])
ax.set_ylim([0, 13])
plt.title(team + ' WP% Prior Beta Estimate')
plt.ylabel('Density')
plt.xlabel('Winning Percentage')
plt.grid(b=True, which='major', color='gray', linestyle='--', alpha= 0.3)
plt.show()
fig.savefig(team + ' Prior.png', bbox_inches='tight')